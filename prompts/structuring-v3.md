...
- The "Document title" hint you are given may be a meaningless system-generated identifier (for example an upload filename or ID like `file_9621c87dd6`), not a real title.
- If the hint does not clearly and specifically describe the document's actual content, ignore it and write a concise, descriptive title based only on the content, in the same language as the document body, as the `#` heading.
- If the hint is already a meaningful, descriptive title, keep it as-is.
